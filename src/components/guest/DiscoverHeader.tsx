...
export function DiscoverHeader() {
  ...
  return (
    <div className="border-border/60 border-b px-3 pt-2 pb-2.5">
      ...
      {open === "city" && (
        <div className="border-border bg-card scrollbar-hide mt-2 max-h-44 overflow-y-auto rounded-2xl border p-1 shadow-md">
          {CITIES.map((opt) => (
            <button
              key={opt}
              type="button"
      ...
      {open === "when" && (
        <div className="border-border bg-card mt-2 grid grid-cols-2 gap-1.5 rounded-2xl border p-2 shadow-md">
          <div>
            <div className="text-muted-foreground px-1 pb-1 text-[9px] font-medium tracking-[0.18em] uppercase">
              Date
            </div>
            <div className="scrollbar-hide max-h-40 space-y-0.5 overflow-y-auto pr-0.5">
              {DATES.map((d) => (
                <button
                  key={d}
                  type="button"
