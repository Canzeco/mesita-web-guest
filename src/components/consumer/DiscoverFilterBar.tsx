...
// WHAT / WHERE / WHEN discovery filter. Lives as a full-width band directly
// beneath the top chrome row (which now hosts the Swipe/Map/AI-Search tabs).
// Picker state is local + presentational for now — wiring to real results
// lands with the search backend.
...
export function DiscoverFilterBar() {
  ...
  return (
    ...
    // picker dropdowns; z-30 keeps it (and its overlay) above the scrolling
    // discover surface, mirroring the old in-header behavior.
    <div className="relative z-30 px-3 pt-2 pb-1">
      <div className="border-border bg-card/70 flex w-full items-center gap-0.5 rounded-full border p-1 backdrop-blur">
        <button
          type="button"
      ...
      {open && (
        <>
          {/* Backdrop. Sits below the picker dropdown but above everything
              else so a tap outside dismisses it without scrolling or
